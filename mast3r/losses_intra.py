...
from dust3r.losses_intra import BaseCriterion, Criterion, MultiLoss, Sum, ConfLoss
from dust3r.losses_intra import Regr3D as Regr3D_dust3r
from dust3r.utils.geometry import (geotrf, inv, normalize_pointcloud)
from dust3r.inference_intra import get_pred_pts3d
...
class Regr3D (Regr3D_dust3r):
    def __init__(self, criterion, norm_mode='avg_dis', gt_scale=False, opt_fit_gt=False,
                 sky_loss_value=0, max_metric_scale=False, loss_in_log=False):
    ...
    def get_all_pts3d(self, pred1, pred2, dist_clip=None):
        pr_pts1 = get_pred_pts3d(pred1, use_pose=False).clone()
        pr_pts2 = get_pred_pts3d(pred2, use_pose=True).clone()

        return pr_pts1, pr_pts2

    def compute_loss(self, gt1, gt2, pred1, pred2, **kw):
        # gt_pts1, gt_pts2, pred_pts1, pred_pts2, mask1, mask2, sky1, sky2, monitoring = \
        #     self.get_all_pts3d(gt1, gt2, pred1, pred2, **kw)

        gt_pts1, gt_pts2 = gt1['pts3d'], gt2['pts3d_in_other_view']
        pred_pts1, pred_pts2 = self.get_all_pts3d(pred1, pred2)

        # mask1 and mask2 are the whole image
        mask1 = torch.ones_like(gt_pts1[..., 0], dtype=bool)
        mask2 = torch.ones_like(gt_pts2[..., 0], dtype=bool)

        # loss on img1 side
        pred_pts1 = pred_pts1[mask1].detach()
        gt_pts1 = gt_pts1[mask1].detach()
        l1 = self.criterion(pred_pts1, gt_pts1.to(pred_pts1.device))

        # loss on gt2 side
        pred_pts2 = pred_pts2[mask2]
        gt_pts2 = gt_pts2[mask2]
        l2 = self.criterion(pred_pts2, gt_pts2.to(pred_pts2.device))

        self_name = type(self).__name__
        details = {self_name + '_pts3d_1': float(l1.mean()), self_name + '_pts3d_2': float(l2.mean())}
        return Sum((l1, mask1), (l2, mask2)), (details)
...
class InfoNCE(MatchingCriterion):
    ...
    def loss(self, desc1, desc2, valid_matches=None, euc=False):
        ...
        if not (valid_matches.shape == torch.Size([B, N]) and valid_matches.sum() > 0):
            return torch.zeros((B, 1), device=desc1.device, requires_grad=True)
...
class MatchingLoss (Criterion, MultiLoss):
    ...
    def get_matching_descs(self, gt1, gt2, pred1, pred2, **kw):
        ...
        kp_1, kp_2 = kw.get('kp_1', None), kw.get('kp_2', None)

        # (x1, y1), (x2, y2) = gt1['corres'].unbind(-1), gt2['corres'].unbind(-1)
        # use kp_1 which is numpy array shape (N, 2) + change into tensor
        (x1, y1) = torch.from_numpy(kp_1).unsqueeze(0).unbind(-1)
        (x2, y2) = torch.from_numpy(kp_2).unsqueeze(0).unbind(-1)

        valid_matches = torch.ones_like(x1, dtype=bool).to(x1.device)
    ...
    def blockwise_criterion(self, descs1, descs2, confs1, confs2, valid_matches, euc, rng=np.random, shuffle=True):
        ...
        # loss = loss.mean() if loss.numel() > 0 else loss.new_zeros(())
        return loss, details
