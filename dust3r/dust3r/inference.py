...
# TO TEST
def finetune(pairs, model, device, batch_size=8, criterion=None, verbose=True):
    if verbose:
        print(f'>> Inference with model on {len(pairs)} image pairs')
    result = []
    ...
    for i in tqdm.trange(0, len(pairs), batch_size, disable=not verbose):
        res = loss_of_one_batch(collate_with_cat(pairs[i:i + batch_size]), model, criterion, device)
        result.append(to_cpu(res))
