...
def loss_of_one_batch(batch, model, criterion, gt_output, kp_1, kp_2, device, symmetrize_batch=False, use_amp=False, ret=None):
    view1, view2 = batch
    ...
    with torch.cuda.amp.autocast(enabled=bool(use_amp)):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            loss = None
            if criterion:
                gt_pred1 = gt_output['pred1']
                gt_pred2 = gt_output['pred2']
                loss = criterion(gt_pred1, gt_pred2, pred1, pred2, kp_1=kp_1, kp_2=kp_2)
...
# TO TEST
def finetune(pairs, model, gt_output, kp_1, kp_2, device, batch_size=8, criterion=None, verbose=True):
    if verbose:
        print(f'>> Inference with model on {len(pairs)} image pairs')
    result = []
    ...
    for i in tqdm.trange(0, len(pairs), batch_size, disable=not verbose):
        res = loss_of_one_batch(collate_with_cat(pairs[i:i + batch_size]), model, criterion, gt_output, kp_1, kp_2, device)
        # exclude loss from the result
        loss = res.pop('loss')
...
def get_pred_pts3d(pred, use_pose=False):
