...
class PatchEmbedDust3R(PatchEmbed):
    def forward(self, x, **kw):
        ...
        #     x = self.proj(x) 
        # else:
        #     x = self.model.forward_features(self.input_transform(x))[:, 1:].transpose(1, 2) # B C N
        #     x = x.reshape(B, -1, H // self.patch_size[0], W // self.patch_size[1]) # B C H W
        #     C = x.size(1)
        #     x = self.refine_conv(x).reshape(B, C, -1).transpose(1, 2) # B N C
        #     x = self.linear_layer(x).transpose(1, 2).reshape(B, -1, H // self.patch_size[0], W // self.patch_size[1])
